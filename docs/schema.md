...
| `FreeBSD` | The FreeBSD ecosystem encompasses the core system, kernel, and ports. The `name` refers to the name of a package managed by `pkg(8)`, the package manager. This ecosystem string can include `:ports`, indicating that it pertains to issues within FreeBSD ports. Additionally, it may contain an optional `:<RELEASE>` segment, specifying that the issue is specific to a particular FreeBSD release. For core system issues (`:core`), the `ranges[].events` versions correspond to different FreeBSD releases. Similarly, kernel-related issues are denoted by `:kernel`. |
